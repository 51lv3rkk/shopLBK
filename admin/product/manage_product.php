...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Sửa ": "Tạo " ?> Sản Phẩm</h3>
	</div>
	<div class="card-body">
		<form action="" id="product-form">
            ...
            <div class="form-group">
				<label for="status" class="control-label">Tình trạng sản phẩm</label>
                <select name="status" id="status" class="custom-select selevt">
                <option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Mở</option>
                <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Đóng</option>
                </select>
			</div>
            <div class="form-group">
...
<script>
    ...
    function delete_img($path){
        ...
        $.ajax({
            ...
            error:err=>{
                console.log(err)
                alert_toast("Có lỗi xảy ra trong quá trình xoá ảnh","error");
                end_loader()
            },
